---
title: Copilot Features MOC
type: moc
fetched: 2026-07-07
tags: [copilot, moc]
---

# Copilot Features MOC

The full Microsoft 365 Copilot capability surface, organized by where it lives. Each category note links its individual feature notes. Newest capabilities (surfaced in the last ~90 days) are tagged `new: true` in their frontmatter rather than siloed off here.

Looking for "how do I solve *X*"? Start from the [Recipes](../Recipes/README.md) instead, which is organized by the problem rather than the app.

## In-app Copilot

- [Word](Word/README.md) — draft, rewrite, chat over a document, ground on your files
- [Excel](Excel/README.md) — analyze data, formulas, charts, Agent Mode
- [PowerPoint](PowerPoint/README.md) — build decks from a prompt or your work content, generate images
- [Outlook](Outlook/README.md) — summarize threads, draft, coaching, chat across the inbox
- [Teams](Teams/README.md) — meeting recap, in-meeting Copilot, chat catch-up, calls
- [OneNote & Loop](OneNote%20%26%20Loop/README.md) — summarize notes, extract tasks, draft; collaborative pages

## The Copilot app

- [Copilot Chat](Copilot%20Chat/README.md) — work- and web-grounded chat, model modes, files, images, vision
- [Researcher & Analyst](Researcher%20%26%20Analyst/README.md) — the two reasoning agents for deep research and data analysis
- [Pages & Notebooks](Pages%20%26%20Notebooks/README.md) — collaborative canvases and persistent project workspaces
- [Create](Create/README.md) — images, posters, video, and forms from a description
- [Search & Work IQ](Search%20%26%20Work%20IQ/README.md) — semantic search across your tenant and the intelligence layer under it
- [Cowork](Cowork/README.md) — the agentic executor that plans and does multi-step work

## Extend & govern

- [Agents & Agent Builder](Agents%20%26%20Agent%20Builder/README.md) — the Agent Store, no-code building, and Copilot Studio
- [Admin & Governance](Admin%20%26%20Governance/README.md) — the controls, data protection, and licensing tiers worth knowing exist

## What's new this quarter

Capabilities added or expanded in roughly the last 90 days, folded into the categories above:

- [Use Claude and GPT-5.5 in Copilot](Copilot%20Chat/Use%20Claude%20and%20GPT-5.5%20in%20Copilot.md) · [Vision in Copilot](Copilot%20Chat/Vision%20in%20Copilot.md) · [Open Files and PDFs in Chat](Copilot%20Chat/Open%20Files%20and%20PDFs%20in%20Chat.md)
- [Copilot Cowork](Cowork/Copilot%20Cowork.md) · [Work IQ](Search%20%26%20Work%20IQ/Work%20IQ.md) · [Copilot Connectors](Search%20%26%20Work%20IQ/Copilot%20Connectors.md) · [Scheduled Prompts and Long-Running Tasks](Cowork/Scheduled%20Prompts%20and%20Long-Running%20Tasks.md)
- [Turn a Notebook Into Office Files](Pages%20%26%20Notebooks/Turn%20a%20Notebook%20Into%20Office%20Files.md) · [Ground a Deck in Your Work Content](PowerPoint/Ground%20a%20Deck%20in%20Your%20Work%20Content.md) · [Agent Mode in Excel](Excel/Agent%20Mode%20in%20Excel.md)
- [Copilot Edits Your Document](Word/Copilot%20Edits%20Your%20Document.md) · [Audio Overview in Word](Word/Audio%20Overview%20in%20Word.md) · [Generate Images in PowerPoint](PowerPoint/Generate%20Images%20in%20PowerPoint.md)
- [Brand Kit for Visuals](Create/Brand%20Kit%20for%20Visuals.md) · [Copilot Chat Across Your Inbox](Outlook/Copilot%20Chat%20Across%20Your%20Inbox.md)

See [_sources](../_sources.md) for every page behind these notes, with fetch dates for diffing on a re-run.
